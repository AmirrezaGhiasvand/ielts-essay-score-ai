...
const MainForm = ({
  ...
  const [testExamMode, setTestExamMode] = useState(false);
  const {
    register,
    handleSubmit,
    watch,
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            disabled={loadingSample || testExamMode}
            className="w-full flex items-center bg-primary/20 justify-center gap-2 text-xs font-medium text-text/90  border-2 border-dashed border-border hover:border-foreground disabled:hover:border-border hover:bg-primary/70 disabled:hover:bg-primary/20 disabled:hover:text-text hover:text-white duration-350 rounded-lg py-2.5 transition-colors disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed"
          >
            {loadingSample ? (
          ...
          <button
            ...
            onClick={() => {
              handleExamTopic();
              setTestExamMode(true);
            }}
            disabled={loadingSample || testExamMode}
            className="w-full flex items-center bg-primary/20 justify-center gap-2 text-xs font-medium text-text/90  border-2 border-dashed border-border hover:border-foreground hover:bg-primary/70 hover:text-white duration-350 rounded-lg py-2.5 transition-colors disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed disabled:hover:border-border disabled:hover:bg-primary/20 disabled:hover:text-text"
          >
            {t.examTopic}
          </button>
        </div>
        ...
        <div className="space-y-2">
          ...
          <textarea
            {...register("question")}
            disabled={testExamMode}
            spellCheck={false}
            placeholder={t.questionPlaceholder}
            rows={3}
            dir={dirtyFields.question ? "ltr" : langInfo!.dir}
            className="w-full resize-none rounded-lg bg-background border-2 border-border p-2 text-base text-text placeholder:text-text/50 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-colors disabled:text-text/80 disabled:select-none disabled:border-border/10"
          />
          {errors.question && (
        ...
        <div className="flex justify-center items-center gap-2">
          <button
            ...
            onClick={() => setTestExamMode(false)}
          >
            {t.submit}
          </button>
          <button
            type="button"
            disabled={testExamMode ? !dirtyFields.essay : !isDirty}
            ...
            onClick={() => (testExamMode ? reset({ essay: "" }) : reset())}
          >
            {t.clear}
          </button>
        </div>
      </form>
      {testExamMode && <DraggableTimer setTestExamMode={setTestExamMode} />}
    </>
  );
};
